          ...
          <>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3">
                {[ { icon: Moon, label: 'Sleep', value: todayLog?.sleep ? `${todayLog.sleep.hours}h` : '—', goal: `${goals.sleepHours}h`, pct: todayLog?.sleep ? (todayLog.sleep.hours / goals.sleepHours) * 100 : 0, color: 'text-chart-1', bg: 'bg-chart-1/10' }, { icon: Smile, label: 'Mood', value: todayLog?.mood ? MOOD_EMOJIS[todayLog.mood.level].emoji : '—', goal: '', pct: todayLog?.mood ? (todayLog.mood.level / 5) * 100 : 0, color: 'text-chart-4', bg: 'bg-chart-4/10' }, { icon: Droplets, label: 'Water', value: todayLog?.water ? `${todayLog.water.glasses}` : '—', goal: `${goals.waterGlasses}`, pct: todayLog?.water ? (todayLog.water.glasses / goals.waterGlasses) * 100 : 0, color: 'text-chart-3', bg: 'bg-chart-3/10' }, { icon: Dumbbell, label: 'Exercise', value: todayLog?.exercise ? `${todayLog.exercise.durationMinutes}m` : '—', goal: `${goals.exerciseMinutes}m`, pct: todayLog?.exercise ? (todayLog.exercise.durationMinutes / goals.exerciseMinutes) * 100 : 0, color: 'text-chart-2', bg: 'bg-chart-2/10' }, { icon: Apple, label: 'Nutrition', value: todayLog?.nutrition ? [todayLog.nutrition.breakfast, todayLog.nutrition.lunch, todayLog.nutrition.dinner].filter((m) => m === 'healthy').length + '/3' : '—', goal: '', pct: todayLog?.nutrition ? ([todayLog.nutrition.breakfast, todayLog.nutrition.lunch, todayLog.nutrition.dinner].filter((m) => m === 'healthy').length / 3) * 100 : 0, color: 'text-chart-5', bg: 'bg-chart-5/10' }, { icon: Stethoscope, label: 'Symptoms', value: todayLog?.symptoms ? `${todayLog.symptoms.symptoms.length}` : 'None', goal: '', pct: 0, color: 'text-destructive', bg: 'bg-destructive/10' } ].map((item, i) => (
